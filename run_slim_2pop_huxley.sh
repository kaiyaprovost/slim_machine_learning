...
while getopts p:S:I:P:O:m:r:N:g:f:s:w:h:x:y:e:c:i:l:t:B:a: OPTION
do
	case "$OPTION" in
		p)
...
for ((i=1;i<=simulations;i++)); do ## for run in simulations runs
	...
	command="$slimPath -d N=$N -d mu=$mu -d recomb=$recomb -d g_rate=$g_rate -d sweepType=\'$sweepType\' -d selStrength=$selStrength -d outSamp=$outSamp -d N_dif=$N_dif -d nicheX=$nicheX -d nicheY=$nicheY -d nicheFile=\'$nicheFile\' -d competition=$competition -d ibd=$ibd -d spatial=$spatial -d treeSeq=$treeSeq -d subpopCount=$subpopCount -d allPops=$allPops -d textName=\'$filename-$TIMESTAMP.txt\' -d tempName=\'$filename-$TIMESTAMP-$i.temp\' -d treeName=\'$filename-$TIMESTAMP-$i.trees\' $path$script >> slim-$filename-$script-$TIMESTAMP.log 2>&1"
	...
	if [ $treeSeq==1 ]; then
		...
		command2="python /home/kprovost/nas1/Analysis_SLiM/Overlaying_neutral_mutations_klp.py $filename-$TIMESTAMP-$i.trees $mu $TIMESTAMP"
		...
		command3="perl /home/kprovost/nas1/Analysis_SLiM/vcf2MS.pl $filename-$TIMESTAMP-$i-overlaid.vcf $filename-$TIMESTAMP-$i.temp $outSamp"
