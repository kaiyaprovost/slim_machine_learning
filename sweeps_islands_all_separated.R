...
dxydf$midPos = dxydf$starts + 59999
...
tajdf = read.table("/Users/kprovost/Dropbox (AMNH)/Dissertation/CHAPTER2_GENOMES/ANALYSIS/DXY/textfiles/bigtaj.new.may2020.txt",
                   header=T,sep=",",stringsAsFactors = T,fill=T)
tajdf$chr = tajdf$Chr
...
for(row_index in 1000000:nrow(merged)){
...
mergx=merged[,c("Fst","plotorder","species","weighted_recomb","dxymeans")]
mergx=unique(mergx)
...
#pdf("corrplots_fst_rec_dxy_taj.pdf")
...
#                   method="number",main="taj d",#order="hclust",
#                   col=colorRampPalette(RColorBrewer::brewer.pal(11,"RdYlBu"))(200))
dev.off()
...
for (i in 1:length(dxy_lower)){
  for(j in 1:length(dxy_upper)){
    ...
    sweeps_expected = sum((mergx$ranksppDXY < this_dxy_lower) & (mergx$ranksppFST >= fst_upper),na.rm=T)
    islands_expected = sum((mergx$ranksppDXY >= this_dxy_upper) & (mergx$ranksppFST >= fst_upper),na.rm=T)
...
png("expected_sweep_to_island_ratio.png",width=400,height=400)
pal = colorRampPalette(c("white","cyan"))
pal2 = colorRampPalette(c("white","magenta"))
pal3 = colorRampPalette(c("cyan","white","magenta"))
par(mar=c(4,4,0,0))
#cuts=c(-1,-0.5,-0.01,0.01,0.5,1,10,100,1000,10000,100000)
cuts=c(-10,-1,-0.1,-0.01,0,0.01,0.1,1,10,100,1000,10000,100000)
plot(dxy_dxy_raster,xlab="Island Cutoff",ylab="Sweep Cutoff",
     breaks=cuts,
     #col=pal3(length(cuts)+1),
     col=c(rev(pal(8))[5:8],pal2(8)),asp=NA,
     #col=c(rev(pal(3)),pal2(8)),
     las=2,axes=F,interpolate=F,colNA = "grey",
     main="",legend=F)
legend(x=0,y=1,fill=c(rev(pal(8))[5:8],pal2(8)),title="Islands:Sweeps",
       legend=c("2:1","1.1:1","1.01:1","1:1",
                "1:1.01","1:1.1","1:2","1:10","1:100","1:1000","1:10000"),
       bty="n")
...
merged=merge(merged,missingdf,by=c("chr","midPos","species"),all=T)
merged=unique(merged)
write.table(merged,"~/rec_taj_dxy_fst_islswp_missing.temp",row.names=F,quote=F,sep="\t")
