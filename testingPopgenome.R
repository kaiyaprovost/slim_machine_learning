...
GENOME.class <- detail.stats(GENOME.class,biallelic.structure = T)
GENOME.class <- calc.R2(GENOME.class)
GENOME.class <- introgression.stats(GENOME.class) ## only works if outgroup
...
moments_haplotype.counts = rbind(all.moments(unlist(STATS.class@haplotype.counts),order.max=4))
colnames(moments_haplotype.counts) = c("prob_haplotype.counts",
...
moments_minor.allele.freqs = rbind(all.moments(unlist(STATS.class@minor.allele.freqs),order.max=4))
colnames(moments_minor.allele.freqs) = c("prob_minor.allele.freqs",
...
moments_nuc.diversity.within = rbind(all.moments(unlist(STATS.class@nuc.diversity.within),order.max=4))
colnames(moments_nuc.diversity.within) = c("prob_nuc.diversity.within",
...
STATS.class_outtable = cbind(
  ...
  moments_haplotype.counts, ## remove the 0th moment, probability
...
GENOME.class.slide <- linkage.stats(GENOME.class.slide,detail = T) ## lengthy 
GENOME.class.slide <- recomb.stats(GENOME.class.slide) ## lengthy  
GENOME.class.slide <- F_ST.stats(GENOME.class.slide) ## never relevant with one pop
GENOME.class.slide <- diversity.stats(GENOME.class.slide)
GENOME.class.slide <- sweeps.stats(GENOME.class.slide)
GENOME.class.slide <- MKT(GENOME.class.slide) ## only works if mult pops 
GENOME.class.slide <- detail.stats(GENOME.class.slide,biallelic.structure = T)
...
GENOME.class.slide_outtable = cbind( ## this is identical to non-slide and therefore redundant
  get.neutrality(GENOME.class.slide)[[1]],
  get.linkage(GENOME.class.slide)[[1]],
  get.recomb(GENOME.class.slide)[[1]],
  get.diversity(GENOME.class.slide)[[1]]
...
## get stats for MS 
head(get.sum.data(MS.class))
head(get.neutrality(MS.class)[[1]])
head(get.linkage(MS.class)[[1]])
head(get.recomb(MS.class)[[1]])
#head(get.F_ST(MS.class)) ## never relevant with one pop
head(get.diversity(MS.class)[[1]])
...
MS.haplotype.diversity = cbind(unlist(MS.STATS.class@haplotype.diversity))
...
MS.STATS.class@haplotype.counts ## large 
## probability, mean, variance, skewness, kurtosis 
momentsMS_haplotype.counts = t(sapply(1:length(MS.STATS.class@haplotype.counts), function(i) {all.moments(unlist(MS.STATS.class@haplotype.counts[i]),order.max=4)}))
colnames(momentsMS_haplotype.counts) = c("prob_haplotype.counts",
                                       "mean_haplotype.counts",
                                       "var_haplotype.counts",
                                       "skew_haplotype.counts",
                                       "kurt_haplotype.counts")
...
MS.STATS.class@minor.allele.freqs ## large 
momentsMS_minor.allele.freqs = t(sapply(1:length(MS.STATS.class@minor.allele.freqs), function(i) {all.moments(unlist(MS.STATS.class@minor.allele.freqs[i]),order.max=4)}))
colnames(momentsMS_minor.allele.freqs) = c("prob_minor.allele.freqs",
                                         "mean_minor.allele.freqs",
                                         "var_minor.allele.freqs",
                                         "skew_minor.allele.freqs",
                                         "kurt_minor.allele.freqs")
...
MS.STATS.class@linkage.disequilibrium  ## HUGE amount of stuff - not sure how to get
...
MS.STATS.class@nuc.diversity.within ## large
momentsMS_nuc.diversity.within = t(sapply(1:length(MS.STATS.class@nuc.diversity.within), function(i) {all.moments(unlist(MS.STATS.class@nuc.diversity.within[i]),order.max=4)}))
colnames(momentsMS_nuc.diversity.within) = c("prob_nuc.diversity.within",
                                           "mean_nuc.diversity.within",
                                           "var_nuc.diversity.within",
                                           "skew_nuc.diversity.within",
                                           "kurt_nuc.diversity.within")
...
MS.STATS.class_outtable = cbind(
  ...
  momentsMS_haplotype.counts, ## remove the 0th moment, probability
