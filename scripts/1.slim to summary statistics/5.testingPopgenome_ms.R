...
files=c()
path="/Users/kprovost/Dropbox (AMNH)/Dissertation/CHAPTER2_GENOMES/ANALYSIS/called_geno/SPECIES/TEMPS/" ## 2291
setwd(path)
...
path="/Users/kprovost/Dropbox (AMNH)/Dissertation/CHAPTER1_REVIEW/SLIM/runs/" ## 2957
setwd(path)
files = c(files,list.files(path = path,pattern = "*fulltemp$",recursive = TRUE,full.names = T))
files = c(files,list.files(path = path,pattern = "*subsettemp$",recursive = TRUE,full.names = T))
files=unique(files)
    ...
    if(file.exists(msfile)) {
      ...
      if (can_run == TRUE) {
        ...
        if (overwrite != T && file.exists(outfile_text)) {
          ...
          if(isTRUE(class(MS.class)=="try-error")) { next } else { 
            ...
            if (length(get.individuals(MS.class)) != 0) {
              ...
              if(isTRUE(class(MS.class_outtable)=="try-error")) { next } else { 
                ...
                if (doWindow == T) {
                  ...
                  if (n.sites > 1) {
                    ...
                    if(isTRUE(class(MS.class.win)=="try-error")) { next } else { 
                      ...
                      if(isTRUE(class(MS.class_outtable_window)=="try-error")) { next } else { 
