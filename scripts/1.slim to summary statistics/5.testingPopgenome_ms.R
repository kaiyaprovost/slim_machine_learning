...
## set up functions
{
  ...
  runStatsMS = function(MS.class_temp=MS.class, window = F, sum = T, neut = T, link = T, recom = T, fst = T, div = T, mkt = T,
    ...
    if (neut == T) {
      ...
      MS.class_temp <- neutrality.stats(MS.class_temp,detail=T,do.R2=T)
  ...
  readMS.custom = function (file, big.data = FALSE,verbose=FALSE,include.unknown=TRUE) {
    if (!big.data) {
      ...
      test <- readData.custom(
        "SwapMS", SNP.DATA = TRUE, FAST = TRUE, format = "RData", big.data = big.data,include.unknown=include.unknown)
    ...
    if(file.exists(msfile)) {
      ...
      if (can_run == TRUE) {
        ...
        if (overwrite != T && file.exists(outfile_text)) {
          ...
          MS.class = try(readMS.custom(file,big.data = T,verbose = T,include.unknown = T),silent=F)
          ...
          if(isTRUE(class(MS.class)=="try-error")) { 
            ...
            } else {if(verbose==T){print("no individuals found")}}
            rm(MS.class)
          }
        }
        gzip(msfile, overwrite = T)
