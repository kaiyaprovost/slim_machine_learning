    ...
    if(length(temp) <= 6) {
      ...
      if (file.exists(paste(path, "/",filename,"_STATS.txt", sep = "")) && overwrite == F ) {
        ...
        print("STATS AND REGION PANMIXIA")
        {
          ...
          GENOME.class_outtable = cbind(GENOME.class_outtable, momentsGENOME_minor.allele.freqs)
          colnames(GENOME.class_outtable) = make.unique(colnames(GENOME.class_outtable))
        ...
        analyses_string = paste(analyses,collapse = "-")
        
        write.table(
          as.matrix(GENOME.class_outtable),
          paste(path, "/",filename,"_STATS_",analyses_string,".txt", sep = ""),
          sep = "\t",
          row.names = T,
          col.names = T,
          append=F
