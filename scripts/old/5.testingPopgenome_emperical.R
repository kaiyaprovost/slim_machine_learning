...
packages = c("PopGenome", "moments", "R.utils", "moments")
for (p in packages) {
  dynamic_require(p,lib=.libPaths()[2])
}
path="/vz-nas1-active/ProcessedGenomicReads/EVERY_PLATE/ANGSD/VCFS/CRISSALE/"
setwd(path)
      ...
      if(length(lines)>0){
        try({
          ...
          if (overwrite != T &&
            ...
            R.utils::gzip(msfile,overwrite=T,remove=T)
