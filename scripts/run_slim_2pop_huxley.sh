...
for ((i=1;i<=simulations;i++)); do ## for run in simulations runs
	...
	if [ $treeSeq==1 ]; then
		...
		command2="python /home/kprovost/nas2/Analysis_SLiM/Overlaying_neutral_mutations_klp.py $filename-$TIMESTAMP-$i.trees $mu $TIMESTAMP"
		...
		command3="perl /home/kprovost/nas2/Analysis_SLiM/vcf2MS.pl $filename-$TIMESTAMP-$i-overlaid.vcf $filename-$TIMESTAMP-$i.temp $outSamp"
