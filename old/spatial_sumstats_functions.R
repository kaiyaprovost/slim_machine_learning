...
sPCA.dist = function(data, pops.xy, nsamples, cpos=2, cneg=2, plot=T){
  ...
  pops = data[,1]
  DATA = genind(data2,pop=pops,ploidy=1, type="PA")
  
  XY = NULL
  for (i in 1:nrow(pops.xy)){
    row = cbind(as.numeric(pops.xy[i,1]),as.numeric(pops.xy[i,2]))
    for (j in 1:nsamples){
      XY = rbind(XY,row)}}
  
  mySpca.prelim = spca(obj = DATA, xy = XY, type = 7, a=2, dmin=0.1, scannf = FALSE, plot.nb = FALSE)
  ...
  mySpca = spca(obj = DATA, xy = XY, type = 7, a=2, dmin=0.1, scannf = FALSE, plot.nb = FALSE, nfposi = cpos, nfnega = cneg)
  ...
  Spca.scores = cbind(pops,mySpca$li)
  #within distances
  ...
  for (j in 1:nrow(pops.xy)){
    ...
    names(spca.Wit.mean)[j] = paste('Spca.Dmean',(j-1), sep='_')
    spca.Wit.dsd[j] = sd(dist(comp.set))
    names(spca.Wit.dsd)[j] = paste('Spca.Dsd',(j-1), sep='_')
  }
  #between distances
  comp = combn(nrow(pops.xy),2)
  ...
  for (k in 1:ncol(comp)){
    comp.set = rbind(Spca.scores[Spca.scores[,1]==toString(comp[1,k]),2:ncol(Spca.scores)], Spca.scores[Spca.scores[,1]==toString(comp[2,k]),2:ncol(Spca.scores)])
    ...
    names(spca.Bet.mean)[k] = paste('Spca.Dmean',(comp[1,k]-1),(comp[2,k]-1),sep='_')
    spca.Bet.dsd[k] = sd(spca.Bet.dist)
    names(spca.Bet.dsd)[k] = paste('SPCA.Dsd',(comp[1,k]-1),(comp[2,k]-1),sep='_')
  }
