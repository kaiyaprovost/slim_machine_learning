...
## THIS WILL NOT WORK ON VCF IF IN DROPBOX
bigpath = "/home/kprovost/nas2/Analysis_SLiM/FINISHED_SCALED/VCFS/"

pathlist = sapply(
  seq(0, 99),
  FUN = function(x) {
    return(paste(bigpath, "FOLDER-", x, "/", sep = ""))
  }
)
...
for (path in pathlist) {
  ...
  if (do_vcf == T) {
    ...
    numind = length(get.individuals(GENOME.class)[[1]])
  ...
  write.table(
    ...
    paste(path, "Test_emprical_data_1tab-",split,".txt", sep = ""),
    sep = "\t",
    row.names = T,
    col.names = T
  )
